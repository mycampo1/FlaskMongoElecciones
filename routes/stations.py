...
def station(station_id):
    try:
        station = stations_controller.get_by_id(station_id)
...
def update_station(station_id):
    try:
        station = stations_controller.update(station_id, request.get_json())
        return jsonify({
            "message": "Mesa de votación actualizada exitosamente",
            "station": station.__dict__
        }), 200
    except StationDoesNotExist:
        return jsonify({
...
def delete_station(station_id):
    try:
        stations_controller.delete(station_id)
        return jsonify({
            "message": "Mesa de votación borrada exitosamente"
        }), 200
    except StationDoesNotExist:
        return jsonify({
            "error": "Mesa de votación no existe"
